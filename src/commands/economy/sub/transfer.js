...
module.exports = async (self, target, coins) => {
  if (isNaN(coins) || coins <= 0) return "<:Info:1330256387959164928> Please enter a valid amount of coins to transfer";
  if (target.bot) return "<:bot:1247839084030722109> You cannot transfer coins to bots!";
  if (target.id === self.id) return "<:No:1330253494447243355> You cannot transfer coins to self!";

  const userDb = await getUser(self);

  if (userDb.bank < coins) {
    return `<:No:1330253494447243355> Insufficient bank balance! You only have ${userDb.bank}${ECONOMY.CURRENCY} in your bank account.${
