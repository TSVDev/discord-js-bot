...
module.exports = {
  ...
  async messageRun(message, args, data) {
    ...
    // channel
      ...
      if (matched.length == 0) response = `<:No:1330253494447243355> No matching channels found for " ${input}". Make sure you input a valid channel ID or mention.`;
      else if (matched.length > 1) response = `<:Info:1330256387959164928> Multiple channels found for ${input}. Please be more specific.`;
    ...
    // staffadd
      ...
      if (matched.length == 0) response = `<:No:1330253494447243355> No matching roles found for " ${input}". Make sure you input a valid role ID or mention.`;
      else if (matched.length > 1) response = `<:Info:1330256387959164928> Multiple roles found for ${input}. Please be more specific.`;
...
async function setChannel(settings, channel) {
  ...
  if (!channel.permissionsFor(channel.guild.members.me).has(CHANNEL_PERMS)) {
    return `<:Info:1330256387959164928> I need the following permissions in ${channel}\n${parsePermissions(CHANNEL_PERMS)}`;
  }

  settings.suggestions.channel_id = channel.id;
  await settings.save();
  return `<:Yes:1330253737687781436> Suggestions will now be sent to ${channel}`;
}
...
async function setApprovedChannel(settings, channel) {
  ...
  await settings.save();
  return `<:Yes:1330253737687781436> Approved suggestions will now be sent to ${channel}`;
}
...
async function setRejectedChannel(settings, channel) {
  ...
  await settings.save();
  return `<:Yes:1330253737687781436> Rejected suggestions will now be sent to ${channel}`;
}

async function addStaffRole(settings, role) {
  if (settings.suggestions.staff_roles.includes(role.id)) {
    return `<:No:1330253494447243355> \`${role.name}\` is already a staff role`;
  }
  settings.suggestions.staff_roles.push(role.id);
  await settings.save();
  return `<:Yes:1330253737687781436> \`${role.name}\` is now a staff role`;
}

async function removeStaffRole(settings, role) {
  if (!settings.suggestions.staff_roles.includes(role.id)) {
    return `<:No:1330253494447243355> ${role} is not a staff role`;
  }
  settings.suggestions.staff_roles.splice(settings.suggestions.staff_roles.indexOf(role.id), 1);
  await settings.save();
  return `<:Yes:1330253737687781436> \`${role.name}\` is no longer a staff role`;
}
