  ...
  async messageRun(message, args, data) {
    ...
    //
      ...
      if (isNaN(max) || Number.parseInt(max) < 1) {
        return message.safeReply("<:Info:1330256387959164928> Max Lines must be a valid number greater than 0");
      }
...
async function antiAttachments(settings, input) {
  ...
  await settings.save();
  return `<:Yes:1330253737687781436> Messages ${
...
async function antilinks(settings, input) {
  ...
  await settings.save();
  return `<:Yes:1330253737687781436> Messages ${status ? "with links will now be automatically deleted" : "will not be filtered for links now"}`;
}

async function maxLines(settings, input) {
  const lines = Number.parseInt(input);
  if (isNaN(lines)) return "<:Info:1330256387959164928> Please enter a valid number input";

  settings.automod.max_lines = lines;
  await settings.save();
  return `${
    input === 0
      ? "<:Yes:1330253737687781436> Maximum line limit is disabled"
      : `<:Yes:1330253737687781436> Messages longer than \`${input}\` lines will now be automatically deleted`
  }`;
}
