  ...
  async messageRun(message, args, data) {
    ...
    switch (type) {
      ...
      case "rep":
        ...
        break;
      default:
        response = "<:Info:1330256387959164928> Invalid Leaderboard type. Choose either `xp`, `invite`or `rep`";
    }
...
async function getXpLeaderboard({ guild }, author, settings) {
  ...
  if (!settings.stats.enabled) return "<:No:1330253494447243355> The leaderboard is disabled on this server";

  const lb = await getXpLb(guild.id, 10);
  if (lb.length === 0) return "<:No:1330253494447243355> There are no users in the leaderboard";
...
async function getInviteLeaderboard({ guild }, author, settings) {
  ...
  if (!settings.invite.tracking) return "<:No:1330253494447243355> Invite tracking is disabled on this server";
