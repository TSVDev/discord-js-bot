...
module.exports = {
  ...
  async messageRun(message, args, data) {
    ...
    if (!["on", "off"].includes(input)) return message.safeReply("<:Info:1330256387959164928> Invalid status. Value must be `on/off`");
...
async function setStatus(input, settings) {
  ...
  return `<:Yes:1330253737687781436> Configuration saved! Stats Tracking is now ${status ? "enabled" : "disabled"}`;
}
