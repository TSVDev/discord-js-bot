  ...
  async messageRun(message, args) {
    ...
    if (input === "volsup") {
      if (!message.member.roles.cache.has(support)) {
        return message.safeReply("You are missing the 'support' role");
      }
      ...
      if (!message.member.roles.cache.has(sls)) {
        return message.safeReply("You are missing the 'SLS Handler' role");
      }
      ...
      if (!message.member.roles.cache.has(staff)) {
        return message.safeReply("You are missing the 'staff' role");
      }
      ...
      return message.safeReply("Unknown Macro");
    }
