...
async function setStatus({ guild }, input, settings) {
  ...
  if (status) {
    if (!guild.members.me.permissions.has(["ManageGuild", "ManageChannels"])) {
      return "<:Info:1330256387959164928> Oops! I am missing `Manage Server`, `Manage Channels` permission!\nI cannot track invites";
    }
  ...
  return `<:Yes:1330253737687781436> Configuration saved! Invite tracking is now ${status ? "enabled" : "disabled"}`;
}
